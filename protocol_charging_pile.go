...
func (this *Charging_Pile_Protocol) ReadPacket(c *gotcp.Conn) (gotcp.Packet, error) {
	...
	for {
		...
		case protocol.PROTOCOL_REQ_PRICE:
			p := protocol.ParsePrice(pkgbyte, smconn.Charging_Pile.Station_ID)
			smconn.ReadMore = false
		...
		case protocol.PROTOCOL_REP_CHARGING:
			p := protocol.ParseRepCharging(pkgbyte, smconn.Charging_Pile.TransactionID)
			smconn.ReadMore = false
		...
		case protocol.PROTOCOL_REP_UPLOAD_OFFLINE_PACAKGE:
			p := protocol.ParseUploadOfflineData(pkgbyte, smconn.Charging_Pile.Station_ID, smconn.Charging_Pile.DB_ID)
			smconn.ReadMore = false

			return pkg.New_Charging_Pile_Packet(protocol.PROTOCOL_REP_UPLOAD_OFFLINE_PACAKGE, p), nil
